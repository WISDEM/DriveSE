...
class Sph_Hub(object):
    ...
    def compute(self, blade_root_diameter, rotor_rpm, blade_mass, rotor_diameter, blade_length):  
        ...
        if self.debug:
            sys.stderr.write('Sph_Hub: INPUTS BRD {:.1f} m RPM {:.1f} BMass {:.1f} RDiam {:.1f} m BLen {:.1f} m\n'.format(blade_root_diameter, \
...
class Sph_Spinner(object):
    ...
    def __init__(self, blade_number, debug=False):
        super(Sph_Spinner, self).__init__()
...
if __name__ == "__main__":
    ...
    spin = Sph_Spinner(blade_number=3, debug=True)
    ...
    hub = Sph_Hub(blade_number=3, debug=True)
