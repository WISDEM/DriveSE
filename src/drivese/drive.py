...
class NacelleBase(Group):
    ...
    rotor_mass=val=0.0 iotype='in', units='kg', desc='rotor mass')
    rotor_torque=val=0.0 iotype='in', units='N*m', desc='rotor torque at rated power')
    rotor_thrust=val=0.0 iotype='in', units='N', desc='maximum rotor thrust')
    rotor_speed=val=0.0 iotype='in', units='rpm', desc='rotor speed at rated')
    machine_rating=val=0.0 iotype='in', units='kW', desc='machine rating of generator')
    gear_ratio=val=0.0 iotype='in', desc='overall gearbox ratio')
    tower_top_diameter=val=0.0 iotype='in', units='m', desc='diameter of tower top')
    rotor_bending_moment=val=0.0 iotype='in', units='N*m', desc='maximum aerodynamic bending moment')

    # parameters
    drivetrain_design=val='geared', ('geared', 'single_stage', 'multi_drive', 'pm_direct_drive'), iotype='in')
    crane=val=Falseiotype='in', desc='flag for presence of crane', deriv_ignore=True)
    bevel=val=00, iotype='in', desc='Flag for the presence of a bevel stage - 1 if present, 0 if not')
    gear_configuration=val=iotype='in', desc='tring that represents the configuration of the gearbox (stage number and types)')

    # outputs
    nacelle_mass=val=0.0 iotype='out', units='kg', desc='nacelle mass')
    nacelle_cm=val=np.array([])iotype='out', units='m', desc='center of mass of nacelle from tower top in yaw-aligned coordinate system')
    nacelle_I=val=np.array([])iotype='out', units='kg*m**2', desc='mass moments of inertia for nacelle [Ixx, Iyy, Izz, Ixy, Ixz, Iyz] about its center of mass')
    low_speed_shaft_mass=val=0.0 iotype='out', units='kg', desc='component mass')
    main_bearing_mass=val=0.0 iotype='out', units='kg', desc='component mass')
    second_bearing_mass=val=0.0 iotype='out', units='kg', desc='component mass')
    gearbox_mass=val=0.0 iotype='out', units='kg', desc='component mass')
    high_speed_side_mass=val=0.0 iotype='out', units='kg', desc='component mass')
    generator_mass=val=0.0 iotype='out', units='kg', desc='component mass')
    bedplate_mass=val=0.0 iotype='out', units='kg', desc='component mass')
...
class Drive3pt(Group):
    ...
    bedplate_mass=val=0.0 iotype='out', units='kg', desc='component mass')
    yaw_system_mass=val=0.0 iotype='out', units='kg', desc='component mass')
    transformer_mass=val=0.0 iotype='out', units='kg', desc='component mass')

    # outputs for hub CM calcuations kept because spinner and others stil use
    MB1_location=val=np.array([])iotype='out', units='m', desc='center of mass of main bearing in [x,y,z] for an arbitrary coordinate system')

    # new variables
    rotor_bending_moment_x=val=0.0 iotype='in', units='N*m', desc='The bending moment about the x axis')
    rotor_bending_moment_y=val=0.0 iotype='in', units='N*m', desc='The bending moment about the y axis')
    rotor_bending_moment_z=val=0.0 iotype='in', units='N*m', desc='The bending moment about the z axis')
    rotor_force_x=val=0.0 iotype='in', units='N', desc='The force along the x axis applied at hub center')
    rotor_force_y=val=0.0 iotype='in', units='N', desc='The force along the y axis applied at hub center')
    rotor_force_z=val=0.0 iotype='in', units='N', desc='The force along the z axis applied at hub center')
    shaft_angle=val=0.0 iotype='in', units='deg', desc='Angle of the LSS inclindation with respect to the horizontal')
    blade_root_diameter=val=0.0 iotype='in', units='m', desc='blade root diameter')
    shaft_ratio=val=0.0 iotype='in', desc='Ratio of inner diameter to outer diameter.  Leave zero for solid LSS')
    rotor_speed=val=0.0 iotype='in', units='rpm', desc='Speed of rotor at rated power')
    ratio_type=val=iotype='in', desc='optimal or empirical stage ratios')
    shaft_type=val=iotype='in', desc='normal or short shaft length')
    uptower_transformer=val=Falseiotype='in', desc='Boolean stating if transformer is uptower')
    shrink_disc_mass=val=0.0 iotype='in', desc='Mass of the shrink disc')
    carrier_mass=val=0.0 iotype='in', units='kg', desc='Carrier mass')
    flange_length=val=0.0 iotype='in', units='m', desc='flange length')
    L_rb=val=0.0 iotype='in', units='m', desc='distance between hub center and upwind main bearing')
    overhang=val=0.0 iotype='in', units='m', desc='Overhang distance')
    gearbox_cm=val=0.0, iotype='in', units='m', desc='distance from tower-top center to gearbox cm--negative for upwind')
    hss_length=val=0.0 iotype='in', units='m', desc='optional high speed shaft length determined by user')

    # new parameters
    Np=val=np.array([])np.array([0.0, 0.0, 0.0, ]), iotype='in', desc='number of planets in each stage')
    mb1Type=val='SRB', ('CARB', 'TRB1', 'TRB2', 'SRB', 'CRB', 'RB'), iotype='in', desc='Main bearing type')
    mb2Type=val='SRB', ('CARB', 'TRB1', 'TRB2', 'SRB', 'CRB', 'RB'), iotype='in', desc='Second bearing type')

    # Fatigue Parameters
    check_fatigue=val=0, (0, 1, 2), iotype='in', desc='turns on and off fatigue check. 0 if no fatigue check, 1 if unknown loads, 2 if known loads')
    fatigue_exponent=val=0.0 iotype='in', desc='fatigue exponent of material')
    S_ut=val=0.0700e6, iotype='in', units='Pa', desc='ultimate tensile strength of shaft material')
    weibull_A=val=0.0 iotype='in', desc='weibull scale parameter "A" of 10-minute windspeed probability distribution')
    weibull_k=val=0.0 iotype='in', desc='weibull shape parameter "k" of 10-minute windspeed probability distribution')
    blade_number=val=3, iotype='in', desc='number of blades on rotor, 2 or 3')
    cut_in=val=0.0 iotype='in', units='m/s', desc='cut-in windspeed')
    cut_out=val=0.0 iotype='in', units='m/s', desc='cut-out windspeed')
    Vrated=val=0.0 iotype='in', units='m/s', desc='rated windspeed')
    T_life=val=0.0 iotype='in', units='yr', desc='cut-in windspeed')
    IEC_Class=val='A', ('A', 'B', 'C'), iotype='in', desc='IEC class letter: A, B, or C')
    DrivetrainEfficiency=val=0.0 iotype='in', desc='overall drivettrain efficiency')
    availability=val=0.0.95, iotype='in', desc='turbine availability')

    # for use if check_fatigue  = 2:
    rotor_thrust_distribution=val=np.array([])iotype='in', units='N', desc='thrust distribution across turbine life')
    rotor_thrust_count=val=np.array([])iotype='in', desc='corresponding cycle-count array for thrust distribution')
    rotor_Fy_distribution=val=np.array([])iotype='in', units='N', desc='Fy distribution across turbine life')
    rotor_Fy_count=val=np.array([])iotype='in', desc='corresponding cycle-count array for Fy distribution')
    rotor_Fz_distribution=val=np.array([])iotype='in', units='N', desc='Fz distribution across turbine life')
    rotor_Fz_count=val=np.array([])iotype='in', desc='corresponding cycle-count array for Fz distribution')
    rotor_torque_distribution=val=np.array([])iotype='in', units='N*m', desc='torque distribution across turbine life')
    rotor_torque_count=val=np.array([])iotype='in', desc='corresponding cycle-count array for torque distribution')
    rotor_My_distribution=val=np.array([])iotype='in', units='N*m', desc='My distribution across turbine life')
    rotor_My_count=val=np.array([])iotype='in', desc='corresponding cycle-count array for My distribution')
    rotor_Mz_distribution=val=np.array([])iotype='in', units='N*m', desc='Mz distribution across turbine life')
    rotor_Mz_count=val=np.array([])iotype='in', desc='corresponding cycle-count array for Mz distribution')
    ...
    def configure(self):
        ...
        # workflow
        self.driver.workflow.add(['above_yaw_massAdder', 'nacelleSystem', 'lowSpeedShaft', 'mainBearing',
                                 'secondBearing', 'gearbox', 'highSpeedSide', 'transformer', 'generator', 'bedplate', 'yawSystem', 'rna'])
        ...
        self.connect('rotor_bending_moment_x', [
                     'lowSpeedShaft.rotor_bending_moment_x'])
        self.connect('rotor_bending_moment_y', [
                     'bedplate.rotor_bending_moment_y', 'lowSpeedShaft.rotor_bending_moment_y'])
        self.connect('rotor_bending_moment_z',
                     'lowSpeedShaft.rotor_bending_moment_z')
        self.connect('rotor_force_x', 'lowSpeedShaft.rotor_force_x')
        self.connect('rotor_force_y', 'lowSpeedShaft.rotor_force_y')
        self.connect('rotor_force_z', [
                     'bedplate.rotor_force_z', 'lowSpeedShaft.rotor_force_z'])
        # Need to address internal torque calculations...
        self.connect('rotor_torque', [
                     'gearbox.rotor_torque', 'highSpeedSide.rotor_torque'])
        self.connect('rotor_mass', [
                     'bedplate.rotor_mass', 'lowSpeedShaft.rotor_mass', 'transformer.rotor_mass'])
        self.connect('rotor_thrust', 'yawSystem.rotor_thrust')
        self.connect('tower_top_diameter', [
                     'bedplate.tower_top_diameter', 'yawSystem.tower_top_diameter'])
        self.connect('machine_rating', ['bedplate.machine_rating', 'generator.machine_rating',
                     'above_yaw_massAdder.machine_rating', 'lowSpeedShaft.machine_rating', 'transformer.machine_rating'])
        self.connect('drivetrain_design', 'generator.drivetrain_design')
        self.connect('gear_ratio', [
                     'gearbox.gear_ratio', 'generator.gear_ratio', 'highSpeedSide.gear_ratio'])
        ...
        self.connect(
            'mb1Type', ['mainBearing.bearing_type', 'lowSpeedShaft.mb1Type'])
        self.connect(
            'mb2Type', ['secondBearing.bearing_type', 'lowSpeedShaft.mb2Type'])
        self.connect('Np', 'gearbox.Np')
        self.connect('ratio_type', 'gearbox.ratio_type')
        self.connect('shaft_type', 'gearbox.shaft_type')
        self.connect('flange_length', [
                     'bedplate.flange_length', 'lowSpeedShaft.flange_length'])
        self.connect(
            'overhang', ['lowSpeedShaft.overhang', 'bedplate.overhang'])
        ...
        self.connect('DrivetrainEfficiency',
                     'lowSpeedShaft.DrivetrainEfficiency')
        self.connect('rotor_speed', [
                     'lowSpeedShaft.rotor_freq', 'generator.rotor_speed'])
        self.connect('rotor_thrust_distribution',
                     'lowSpeedShaft.rotor_thrust_distribution')
        self.connect('rotor_thrust_count', 'lowSpeedShaft.rotor_thrust_count')
        self.connect('rotor_Fy_distribution',
                     'lowSpeedShaft.rotor_Fy_distribution')
        self.connect('rotor_Fy_count', 'lowSpeedShaft.rotor_Fy_count')
        self.connect('rotor_Fz_distribution',
                     'lowSpeedShaft.rotor_Fz_distribution')
        self.connect('rotor_Fz_count', 'lowSpeedShaft.rotor_Fz_count')
        self.connect('rotor_torque_distribution',
                     'lowSpeedShaft.rotor_torque_distribution')
        self.connect('rotor_torque_count', 'lowSpeedShaft.rotor_torque_count')
        self.connect('rotor_My_distribution',
                     'lowSpeedShaft.rotor_My_distribution')
        self.connect('rotor_My_count', 'lowSpeedShaft.rotor_My_count')
        self.connect('rotor_Mz_distribution',
                     'lowSpeedShaft.rotor_Mz_distribution')
        ...
        # connect components
        self.connect('lowSpeedShaft.design_torque', [
                     'mainBearing.lss_design_torque', 'secondBearing.lss_design_torque'])
        self.connect('lowSpeedShaft.diameter1', [
                     'mainBearing.lss_diameter', 'highSpeedSide.lss_diameter'])
        ...
        self.connect('lowSpeedShaft.bearing_mass1',
                     ['mainBearing.bearing_mass'])
        self.connect('lowSpeedShaft.bearing_mass2',
                     ['secondBearing.bearing_mass'])
        self.connect('lowSpeedShaft.mass', [
                     'bedplate.lss_mass', 'above_yaw_massAdder.lss_mass', 'nacelleSystem.lss_mass', 'low_speed_shaft_mass'])
        self.connect('mainBearing.mass', [
                     'bedplate.mb1_mass', 'above_yaw_massAdder.main_bearing_mass', 'nacelleSystem.main_bearing_mass', 'main_bearing_mass'])
        self.connect('secondBearing.mass', [
                     'bedplate.mb2_mass', 'above_yaw_massAdder.second_bearing_mass', 'nacelleSystem.second_bearing_mass', 'second_bearing_mass'])
        self.connect('gearbox.mass', ['lowSpeedShaft.gearbox_mass',
                     'above_yaw_massAdder.gearbox_mass', 'nacelleSystem.gearbox_mass', 'gearbox_mass'])
        self.connect('highSpeedSide.mass', [
                     'bedplate.hss_mass', 'above_yaw_massAdder.hss_mass', 'nacelleSystem.hss_mass', 'high_speed_side_mass'])
        self.connect('generator.mass', [
                     'bedplate.generator_mass', 'above_yaw_massAdder.generator_mass', 'nacelleSystem.generator_mass', 'generator_mass'])
        self.connect('bedplate.mass', [
                     'above_yaw_massAdder.bedplate_mass', 'nacelleSystem.bedplate_mass', 'bedplate_mass'])
        self.connect('transformer.mass', [
                     'above_yaw_massAdder.transformer_mass', 'transformer_mass'])
        self.connect('above_yaw_massAdder.mainframe_mass',
                     'nacelleSystem.mainframe_mass')
        self.connect('yawSystem.mass', [
                     'nacelleSystem.yawMass', 'yaw_system_mass'])
        self.connect('above_yaw_massAdder.above_yaw_mass', [
                     'yawSystem.above_yaw_mass', 'nacelleSystem.above_yaw_mass'])

        self.connect('lowSpeedShaft.bearing_location2',
                     'secondBearing.location')
        ...
        self.connect('highSpeedSide.cm', [
                     'nacelleSystem.hss_cm', 'generator.highSpeedSide_cm'])
        ...
        self.connect('gearbox.cm', [
                     'lowSpeedShaft.gearbox_cm', 'highSpeedSide.gearbox_cm'])
        self.connect('gearbox.length', [
                     'lowSpeedShaft.gearbox_length', 'highSpeedSide.gearbox_length'])
        ...
        self.connect('uptower_transformer', [
                     'bedplate.uptower_transformer', 'transformer.uptower_transformer'])
        ...
        self.connect('transformer.mass', [
                     'bedplate.transformer_mass', 'nacelleSystem.transformer_mass'])
...
class Drive4pt(Group):
    ...
    rotor_force_z=val=0.0 iotype='in', units='N', desc='The force along the z axis applied at hub center')
    blade_root_diameter=val=0.0 iotype='in', units='m', desc='blade root diameter')
    shaft_angle=val=0.0 iotype='in', units='deg', desc='Angle of the LSS inclindation with respect to the horizontal')  # Bedplate tilting angle
    shaft_ratio=val=0.0 iotype='in', desc='Ratio of inner diameter to outer diameter.  Leave zero for solid LSS')
    rotor_speed=val=0.0 iotype='in', units='rpm', desc='Speed of rotor at rated power')
    flange_length=val=0.0 iotype='in', units='m', desc='flange length')
    overhang=val=0.0 iotype='in', units='m', desc='Overhang distance')
    ...
    mb2Type=val='SRB', ('CARB', 'TRB1', 'TRB2', 'SRB', 'CRB', 'RB'), iotype='in', desc='Second bearing type')
    L_rb=val=0.0 iotype='in', units='m', desc='distance between hub center and upwind main bearing')
    hss_length=val=0.0 iotype='in', units='m', desc='high speed shaft length determined by user. Default 0.5m')

    # fatigue check
    check_fatigue=val=0, (0, 1, 2), iotype='in', desc='turns on and off fatigue check. 0 if no fatigue check, 1 if unknown loads, 2 if known loads')
    fatigue_exponent=val=0.0 iotype='in', desc='fatigue exponent of shaft material')
    S_ut=val=0.0.0e6, iotype='in', units='Pa', desc='ultimate tensile strength of shaft material')
    weibull_A=val=0.0 iotype='in', desc='weibull scale parameter "A" of 10-minute windspeed probability distribution')
    weibull_k=val=0.0 iotype='in', desc='weibull shape parameter "k" of 10-minute windspeed probability distribution')
    blade_number=val=3, iotype='in', desc='number of blades on rotor, 2 or 3')
    cut_in=val=0.0 iotype='in', units='m/s', desc='cut-in windspeed')
    ...
    def configure(self):
        ...
        # workflow
        self.driver.workflow.add(['above_yaw_massAdder', 'nacelleSystem', 'lowSpeedShaft', 'mainBearing',
                                 'secondBearing', 'gearbox', 'highSpeedSide', 'generator', 'bedplate', 'yawSystem', 'transformer', 'rna'])
        ...
        self.connect('lowSpeedShaft.length', 'bedplate.lss_length')
        self.connect('gearbox.length', [
                     'bedplate.gbx_length', 'highSpeedSide.gearbox_length', 'lowSpeedShaft.gearbox_length'])
        self.connect('gearbox.height', ['highSpeedSide.gearbox_height'])
        self.connect('bedplate.height', 'yawSystem.bedplate_height')
        self.connect('bedplate.length', 'above_yaw_massAdder.bedplate_length')
        self.connect('bedplate.width', 'above_yaw_massAdder.bedplate_width')
        ...
        self.connect('gearbox.cm', ['nacelleSystem.gearbox_cm',
                     'highSpeedSide.gearbox_cm', 'lowSpeedShaft.gearbox_cm'])
        self.connect('highSpeedSide.cm', [
                     'nacelleSystem.hss_cm', 'generator.highSpeedSide_cm'])
        self.connect('highSpeedSide.length', 'generator.highSpeedSide_length')
...
#     # parameters
#     rotor_bending_moment = val=0.0 iotype='in', units='N*m', desc='rotor aerodynamic bending moment')
#     shaft_angle = val=0.0 iotype='in', units='deg', desc='Angle of the LSS inclindation with respect to the horizontal')
#     shaft_length = val=0.0 iotype='in', units='m', desc='length of low speed shaft')
#     shaftD1 = val=0.0 iotype='in', desc='Fraction of LSS distance from gearbox to downwind main bearing')
#     shaftD2 = val=0.0 iotype='in', desc='raction of LSS distance from gearbox to upwind main bearing')
#     shaft_ratio = val=0.0 iotype='in', desc='Ratio of inner diameter to outer diameter.  Leave zero for solid LSS')
#     rotor_speed = val=0.0 iotype='in', units='rpm', desc='Speed of rotor at rated power')
#     #gbxPower = val=0.0 iotype='in', units='kW', desc='gearbox rated power')
#     #eff = val=0.0 iotype='in', desc='Fraction of LSS distance from gearbox to downwind main bearing')
#     Np = val=np.array([])np.array([0.0,0.0,0.0,]), iotype='in', desc='number of planets in each stage')
#     ratio_type=val=iotype='in', desc='optimal or empirical stage ratios')
#     shaft_type = val=iotype='in', desc='normal or short shaft length')
# uptower_transformer = val=Falseiotype='in', desc='Boolean stating if
# transformer is uptower')
...
#         # workflow
# self.driver.workflow.add(['above_yaw_massAdder', 'nacelleSystem',
# 'lowSpeedShaft', 'mainBearing', 'secondBearing', 'gearbox',
# 'highSpeedSide', 'generator', 'bedplate', 'yawSystem'])
...
# 'lowSpeedShaft.rotor_bending_moment')

# self.connect('rotor_torque', ['lowSpeedShaft.rotor_torque',
# 'gearbox.rotor_torque', 'highSpeedSide.rotor_torque'])

#         self.connect('rotor_mass', 'lowSpeedShaft.rotor_mass')
# self.connect('rotor_speed', ['mainBearing.rotor_speed',
# 'secondBearing.rotor_speed'])
...
# 'above_yaw_massAdder.machine_rating', 'lowSpeedShaft.machine_rating'])
...
def nacelle_example_5MW_baseline_3pt():
    ...
    for i in range(len(count)):
        standard[i]=.11 * 2.5 * .28 * 13.4 * \
            (log10(288984470) - log10(count[i])) + 0.18
    ...
    nace.rotor_thrust_count=np.logspace(
        log10(328), log10(288984470), endpoint=True, num=100)
    ...
    nace.rotor_torque_distribution=standard.copy() * 0.45 * p_o * \
                                                 (R)**2 * Mx_factor
    nace.rotor_torque_count=nace.rotor_thrust_count.copy()
    nace.rotor_My_distribution=standard.copy() * 0.33 * p_o * .8 * \
                                             (R)**2 * My_factor
    nace.rotor_My_count=nace.rotor_thrust_count.copy()
    nace.rotor_Mz_distribution=standard.copy() * 0.33 * p_o * .8 * \
                                             (R)**2 * Mz_factor
...
def nacelle_example_5MW_baseline_4pt():
    ...
    nace.machine_rating=5000.0
    nace.DrivetrainEfficiency=0.95
    nace.rotor_torque=1.5 * (nace.machine_rating * 1000 / nace.DrivetrainEfficiency) / \
